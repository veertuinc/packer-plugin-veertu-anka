...
* `vm_name` (String) The name for the VM that is created. One is generated using the source_vm_name if not provided (`{{ source_vm_name }}-{10RandomChars}`).
...
* `remote` (String) The registry name (will use your default configuration if not set). This takes priority in Anka 3 and `registry-path` will be ignored.
