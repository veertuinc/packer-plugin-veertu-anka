...
* `disk_size` (String) The size in "[0-9]+G" format, defaults to `40G`.
  ...
  > We will automatically resize the internal disk for you by executing `diskutil apfs resizeContainer` inside of the VM. The plugin resolves the APFS container from `disk0s2`, `diskutil info /`, or `diskutil apfs list`.
