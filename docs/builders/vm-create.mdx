...
* `installer_app` (String) The path to a macOS installer. This process takes about 20 minutes.

* `type` (String) Must be `veertu-anka-vm-create`.

#### Optional Configuration

* `vm_name` (String) The name for the VM that is created. One is generated with installer_app data if not provided (`anka-packer-base-{{ installer_app.OSVersion }}-{{ installer_app.BundlerVersion }}`).
